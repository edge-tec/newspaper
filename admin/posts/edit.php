    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($title) || empty($category_id) || empty($content)) {
            ...
            if (!isset($_SESSION['error'])) {
                $stmt = $pdo->prepare("UPDATE posts SET title=?, slug=?, short_description=?, content=?, image=?, category_id=?, breaking_news=?, status=? WHERE id=?");
                if ($stmt->execute([$title, $slug, $short_description, $content, $imagePath, $category_id, $breaking_news, $status, $id])) {
    ...
    <div class="row">
        <div class="col-md-8">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="content" name="content" rows="15" required><?php echo h($post['content']); ?></textarea>
            </div>
        </div>
        ...
        <div class="col-md-4">
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="breaking_news" name="breaking_news" value="1" <?php echo $post['breaking_news'] ? 'checked' : ''; ?>>
                ...
                <div class="card-body">
                    <select class="form-select" id="category_id" name="category_id" required>
                        ...
                        <?php foreach ($categories as $category): ?>
                            <option value="<?php echo $category['id']; ?>" <?php echo $post['category_id'] == $category['id'] ? 'selected' : ''; ?>><?php echo h($category['name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <?php if ($post['image']): ?>
                        <div class="mb-2">
                            <img src="<?php echo SITE_URL . '/' . $post['image']; ?>" class="img-fluid" alt="Current Image">
                        </div>
                    <?php endif; ?>
